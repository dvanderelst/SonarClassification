...
In this paper, we directly evaluate the hypothesis that sonar provides bats inherently with a low dimensional, yet sufficiently rich, description of the world.  We test this hypothesis in two steps. In a first step, we generate a large collection of synthetic echo sequences. We filter these sequences using a model of the bat's auditory periphery \citep{Wiegrebe2008}. The synthetic echo sequences differ greatly in complexity. Nevertheless, we show that their resulting cochlear output can be mapped into a lower dimensional space. In a second step, we show that echoes collected in real bat habitats can be mapped into the same lower dimensional space, without loss of information. We do this by demonstrating that the low dimensional feature space allows recognizing locations in real habitats. 
...
\todo[inline]{Should we talk about the fact that the PCA model fits well across templates generated with different number of reflectors? This seems to depend on the echo having at least one strong (aka nearby) reflector.}
...
\begin{figure}
	\centering
	\includegraphics[width=1\linewidth]{../results/templates}
...
%Together these two findings lend additional credibility to our hypothesis that bats could recognize locations using their acoustic signature, operationalized as templates. Before, we demonstrated that templates are highly specific to locations and directions. Here, we show that storing such templates does not require extensive neural hardware. Small neural networks can be used to store many (6000-7000) templates. Moreover, this is feasible because templates can effectively be compressed, which we demonstrated here by employing PCA and neural networks of a limited size.

%It is well know that even small, shallow, neural networks are remarkably good at approximating potentially very complex function, even to the extent that it becomes valid to ask why they so powerful. \citet{Lin2017a} suggested that most datasets we are interested in originate from some simple physical law or some generative model with relatively
%few free parameters. In the case of templates, the physical laws that generate them are quite straightforward. In addition, it can be assumed that the structure of natural environments can be described by generative models. Therefore, the ideas put forward by \citet{Lin2017a} might explain, on an abstract level why templates should not require extensive neural hardware to encode.
