...
pca_model_echoes = PCA(n_components=settings.n_components)
...
pca_model_templates = PCA(n_components=settings.n_components)
pca_model_templates.fit_transform(synthetic_templates)
...
synthetic_templates_transformed = pca_model_templates.transform(synthetic_templates)
synthetic_templates_reconstructed = pca_model_templates.inverse_transform(synthetic_templates_transformed)

empirical_templates_transformed = pca_model_templates.transform(empirical_templates)
empirical_templates_reconstructed = pca_model_templates.inverse_transform(empirical_templates_transformed)

print('---> GET CORRELATIONS')


x = synthetic_templates_reconstructed.flatten()
...
x = empirical_templates_reconstructed.flatten()
...
print('---> SAVE DATA')
misc.pickle_save(settings.pca_file, pca_model_templates)
numpy.savez_compressed(settings.scaled_synthetic_templates_file, synthetic_templates=synthetic_templates)
numpy.savez_compressed(settings.reconstructed_synthetic_templates_file, synthetic_templates_reconstructed=synthetic_templates_reconstructed)
...
pyplot.plot(cummmulative_templates / cummmulative_echoes)
#pyplot.xscale('symlog')
pyplot.show()
